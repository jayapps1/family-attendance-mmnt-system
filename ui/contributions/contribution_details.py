...
def contribution_details(app, obligation):
    def show(row):
        ...
        for key in ('family_number', 'affiliation_type', 'eligibility_label', 'contribution', 'amount_due', 'total_paid', 'outstanding'):
            ttk.Label(card.body, text=key.replace('_',' ').title() + ': ' + display(row[key]), style='Card.TLabel').pack(anchor='w', pady=6)
        StatusBadge(card.body, row['status'] or 'No payments').pack(anchor='w', pady=8)
        def refreshed(_):
            dialog.destroy(); app.refresh(); contribution_details(app, row)
        ttk.Button(card.body, text='Not eligible' if not row['eligible'] else 'PAID IN FULL' if row['outstanding'] == 0 else 'Record Payment', state='normal' if row['can_pay'] else 'disabled', style='PrimaryButton.TButton', command=lambda: payment_form(app, row, refreshed)).pack(anchor='w', pady=8)
        def history():
            from ui.contributions.payment_history import PaymentHistory
            dialog.destroy(); app.show(lambda a: PaymentHistory(a, row, return_to=lambda:contribution_details(app,row)))
